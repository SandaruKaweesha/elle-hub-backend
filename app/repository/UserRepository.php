...
class UserRepository{
    ...
    public function existsByEmail(string $email): bool  {
        $sql = "SELECT COUNT(*) FROM users WHERE LOWER(TRIM(email)) = LOWER(TRIM(:email))";
    ...
    public function existsByContactNumber(string $contactNumber): bool {
        ...
        $last9 = strlen($cleanNumber) >= 9 ? substr($cleanNumber, -9) : $cleanNumber;

        if (empty($last9)) {
            return false;
        }

        $tables = ['teams', 'organizers', 'sponsors', 'referees', 'playgrounds'];
        foreach ($tables as $table) {
            try {
                $sql = "SELECT contact_number FROM `$table` WHERE contact_number IS NOT NULL AND contact_number != ''";
                $stmt = $this->connection->prepare($sql);
                $stmt->execute();
                $rows = $stmt->fetchAll(PDO::FETCH_COLUMN);
                foreach ($rows as $row) {
                    $rowClean = preg_replace('/[^0-9]/', '', (string)$row);
                    if (!empty($rowClean)) {
                        $rowLast9 = strlen($rowClean) >= 9 ? substr($rowClean, -9) : $rowClean;
                        if ($rowLast9 === $last9) {
                            return true;
                        }
                    }
                ...
                // Table or column might not exist in edge cases
                continue;
            }
        }
        return false;
...
//    Check the Email is exsiting
    ...
    public function findAll(): array
    {
        $sql = "SELECT u.*,
                       ...
                       t.team_name, t.rating, t.points, t.matches_played, t.wins, t.losses, t.win_rate, t.rank_position, t.district,
                       o.organization_name,
